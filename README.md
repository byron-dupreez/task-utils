...
### 4.0.14
- Updated  `core-functions` dependency to 2.0.18
...
- Updated `core-functions` dependency to 2.0.16
...
### 4.0.9
- Changes to `tasks` module:
  - Changed `parent` property of `Task` class back to non-writable & non-configurable
  - Changed `detachSubTask` method to NOT set parent of a detached task to undefined
