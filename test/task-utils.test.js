...
test('reviveTasks - Scenario 3: Active task definitions excludes task 1 & 4, so task 1 & 4 must become unusable', t => {
  const tasksByName = {arb1: 'Arbitrary 1'};
  ...
  const opts = undefined;
  const activeAndUnusableTasks = checkReviveTasks(t, tasksByName, [taskDef2, taskDef3], opts, desc, origTaskLikes, expectedNames);
